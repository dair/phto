...
static fs::path tempDbPath(const std::string& suffix = "") {
  return fs::temp_directory_path() /
         ("db_test_" + suffix + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()) + ".db");
}
...
public:
  ...
  void testAddDuplicate() {
    m_db->addFile("f1", "photo.jpg", 1024, "jpg");
    CPPUNIT_ASSERT_THROW(m_db->addFile("f1", "other.jpg", 512, "jpg"), DatabaseException);
    try {
...
public:
  ...
  void seedData(int fileCount, int tagCount) {
    for (int i = 0; i < fileCount; ++i) {
      m_db->addFile(
        "f" + std::to_string(i), "file" + std::to_string(i) + ".png", static_cast<uint64_t>(i * 100), "png"
      );
    }
