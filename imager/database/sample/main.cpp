...
static int runFileOp(
  Database& db, const std::string& sub, std::vector<std::string> rest, std::optional<Pagination> page
) {
  if (sub == "add") {
    if (rest.size() < 4) {
  ...
  if (sub == "rename") {
    if (rest.size() < 2) {
      std::cerr << "file rename requires: <id> <new-name>\n";
      return 1;
    }
  ...
  if (sub == "get") {
    if (rest.empty()) {
      std::cerr << "file get requires: <id>\n";
      return 1;
    }
    auto f = db.getFile(rest[0]);
    if (!f) {
      std::cerr << "File not found: " << rest[0] << '\n';
      return 1;
    }
    std::cout << "id\tname\tsize\text\n";
    printFile(*f);
...
static int runTagOp(
  ...
  if (sub == "add") {
    if (rest.empty()) {
      std::cerr << "tag add requires: <name>\n";
      return 1;
    }
    db.addTag(rest[0]);
  ...
  if (sub == "del") {
    if (rest.empty()) {
      std::cerr << "tag del requires: <name>\n";
      return 1;
    }
    db.deleteTag(rest[0]);
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (op == "file") {
      if (rest.empty()) {
        std::cerr << "file requires a sub-command\n";
        return 1;
      }
    ...
    if (op == "tag") {
      if (rest.empty()) {
        std::cerr << "tag requires a sub-command\n";
        return 1;
      }
      std::string sub = rest[0];
      rest.erase(rest.begin());
    ...
    if (op == "bind") {
      if (rest.size() < 2) {
        std::cerr << "bind requires: <file-id> <tag-name>\n";
        return 1;
      }
    ...
    if (op == "unbind") {
      if (rest.size() < 2) {
        std::cerr << "unbind requires: <file-id> <tag-name>\n";
        return 1;
      }
    ...
    if (op == "file-tags") {
      if (rest.empty()) {
        std::cerr << "file-tags requires: <file-id>\n";
        return 1;
      }
    ...
    if (op == "tag-files") {
      if (rest.empty()) {
        std::cerr << "tag-files requires at least one <tag>\n";
        return 1;
      }
