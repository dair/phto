...
public:
  ...
  std::vector<db::File> getFilesBySourceAndBaseName(const std::string& sourceDir, const std::string& baseName);
  ...
  std::vector<std::string> getTagsForFile(const std::string& fileId, std::optional<db::Pagination> page = std::nullopt);
