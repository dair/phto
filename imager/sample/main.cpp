...
static PageArgs parsePage(std::vector<std::string>& args) {
  ...
  for (auto it = args.begin(); it != args.end();) {
    if (*it == "--offset" && std::next(it) != args.end()) {
      if (auto v = parseUInt(*std::next(it))) {
        p.offset = *v;
      }
...
static const char* errName(ErrorCode c) {
  switch (c) {
    case ErrorCode::Ok:
      return "Ok";
    case ErrorCode::BrokenFile:
      return "BrokenFile";
    case ErrorCode::DuplicateFile:
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (cmd == "get") {
      if (rest.empty()) {
        std::cerr << "get requires <id>\n";
        return 1;
      }
      auto info = img.getImage(rest[0]);
      if (!info) {
        std::cerr << "Not found: " << rest[0] << '\n';
        return 2;
      }
    ...
    if (cmd == "untag") {
      if (rest.size() < 2) {
        std::cerr << "untag requires <id> <tag>\n";
        return 1;
      }
    ...
    if (cmd == "search") {
      if (rest.empty()) {
        std::cerr << "search requires at least one <tag>\n";
        return 1;
      }
      auto pg = parsePage(rest);
      std::cout << "id\tname\tsize\text\ttags\n";
