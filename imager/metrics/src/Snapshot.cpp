...
static std::string fmtDuration(uint64_t ns) {
    ...
    if (ns < 1'000'000)     return std::to_string(ns / 1'000) + "us";
...
static std::string fmtBytes(uint64_t bytes) {
    if (bytes < 1'024)                     return std::to_string(bytes) + " B";
    if (bytes < 1'024 * 1'024)             return std::to_string(bytes / 1'024) + " KB";
    if (bytes < 1'024ULL * 1'024 * 1'024)  return std::to_string(bytes / (1'024 * 1'024)) + " MB";
    std::ostringstream os;
    os.precision(1);
...
std::string format(const FullSnapshot& snap) {
    ...
    if (snap.histograms.empty()) return out.str();
    ...
    for (const auto& h : snap.histograms) {
        if (h.name == "addimage_total" || h.name == "validate"  ||
            h.name == "hash"           || h.name == "dedup_check" ||
            h.name == "storage_write"  || h.name == "storage_write_root" ||
            h.name == "db_insert"      || h.name == "db_insert_single" ||
            h.name == "mutex_wait")
            printHist(h);
    }
    ...
    out << "\nThread pool:\n";
    for (const auto& h : snap.histograms) {
        if (h.name == "pool_schedule_latency") printHist(h);
    }
    ...
    out << "\nStorage I/O:\n";
    for (const auto& h : snap.histograms) {
        if (h.name == "storage_read_duration") printHist(h);
    }
    ...
    for (const auto& g : snap.gauges) {
        ...
        if (g.name == "blob_bytes_alive")
            out << "  blob_bytes_alive  current=" << detail::fmtBytes(
                static_cast<uint64_t>(g.value > 0 ? g.value : 0)) << "\n";
    }
