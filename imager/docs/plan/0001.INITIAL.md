...
Both formats use the same ISOBMFF container and are handled by a single validator (`MovValidator`). Validation performs container parse, stream probe, and trial decode of one video frame.
...
- `MovValidator` implements `IValidator`, uses libavformat+libavcodec to verify MOV/MP4 container structure and decode one video frame.
...
2. Look up a registered `IValidator` for that extension.
   - If no validator found → return `UnsupportedFormat`.
