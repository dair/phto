...
void MultiDatabase::parallelWriteAll(Op&& op, Compensate&& compensate) {
  ...
  auto compensateAll = [&]() -> coro::Task<void> {
    ...
    for (size_t i = 0; i < n; ++i) {
      if (!succeeded[i]) {
        continue; // 0 = not succeeded
      }
      tasks.push_back([](coro::ThreadPool& pool, db::Database& db, Compensate& comp_, size_t idx) -> coro::Task<void> {
        co_await pool.schedule();
        try {
          comp_(db, idx);
...
void MultiDatabase::parallelWriteAll(Op&& op) {
  parallelWriteAll(
    std::forward<Op>(op), [](db::Database&, size_t) {
    } // no-op compensation
  );
}
...
void MultiDatabase::addFile(const std::string& id, const std::string& name, uint64_t size, const std::string& ext) {
  parallelWriteAll(
...
void MultiDatabase::deleteFile(const std::string& id) {
  ...
  if (!file) {
    throw db::DatabaseException(db::DatabaseErrorCode::NotFound, "File not found: " + id);
  }
...
void MultiDatabase::editFileName(const std::string& id, const std::string& newName) {
  ...
  parallelWriteAll(
    ...
    [&](db::Database& db, size_t) {
      try {
        db.editFileName(id, originalName);
      } catch (...) {}
    }
  );
}
...
std::vector<std::string> MultiDatabase::getTagsForFile(const std::string& fileId, std::optional<db::Pagination> page) {
